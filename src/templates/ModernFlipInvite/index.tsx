...
import { FlowerCorner, MandalaBurst } from "@/components/wedding/DecorativeSvgs";
...
const ModernFlipInvite = ({ data }: TemplateProps) => {
  ...
  return (
    <InvitationShell
      title="Modern Flip Invite"
      brideName={data.brideName}
      groomName={data.groomName}
      overlayClassName="bg-[radial-gradient(circle_at_top,rgba(255,255,255,0.16),transparent_45%),linear-gradient(180deg,rgba(32,28,24,0.92),rgba(16,14,13,0.96))]"
      cardClassName="border-white/15 bg-white/10 text-white"
      darkControls={false}
    >
      <div className="relative min-h-screen bg-[#ede6dc] px-4 py-6 text-stone-900 sm:px-6">
        <MandalaBurst className="absolute right-4 top-24 h-28 w-28 text-white/25" />
        <FlowerCorner className="absolute left-0 top-20 h-28 w-28 text-stone-400/25" />
        <Link
          to="/templates"
            ...
            <aside className="flex flex-col justify-between gap-6 rounded-[2rem] border border-white/60 bg-white/65 p-6 shadow-lg backdrop-blur">
              <div>
                ...
                <p className="mt-3 text-sm leading-7 text-stone-600">
                  A minimal digital invite with cinematic entry, page-turn navigation, countdown, gallery, and RSVP.
                </p>
              </div>
              ...
              <div className="flex items-center gap-3">
                <button
                  ...
                  className="inline-flex min-h-12 flex-1 items-center justify-center gap-2 rounded-full bg-stone-900 px-4 py-3 text-sm font-medium text-white disabled:opacity-40"
                ...
                <button
                  ...
                  className="inline-flex min-h-12 flex-1 items-center justify-center gap-2 rounded-full border border-stone-300 bg-white px-4 py-3 text-sm font-medium text-stone-900 disabled:opacity-40"
